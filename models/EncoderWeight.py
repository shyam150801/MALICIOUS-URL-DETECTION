...
class EncoderWeight:
    def __init__(self, graph_train, graph_test, pattern1train, pattern2train, pattern3train, pattern1test, pattern2test,
        ...
        # Modify the input dimension to accept concatenated input
        input_dim = tf.keras.Input(shape=(4, 1, 250), name='input')
        concatenated_input = tf.keras.layers.Concatenate(axis=1)([input_dim, input_dim, input_dim, input_dim])
        ...
        graph2vec = tf.keras.layers.Dense(200, activation='relu', name='outputgraphvec')(concatenated_input)
    ...
    def train(self):
        ...
        # In the train method, use the concatenated input and labels
        train_history = self.model.fit(
    [np.concatenate((self.graph_train, self.pattern1train, self.pattern2train, self.pattern3train), axis=1)],
    self.y_train, 
    ...
    [np.concatenate((self.graph_test, self.pattern1test, self.pattern2test, self.pattern3test), axis=1)],
    self.y_test, 
    batch_size=self.batch_size,
    verbose=1
)
